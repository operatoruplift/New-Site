import React, { useEffect, useRef } from 'react';

interface Particle {
  x: number;
  y: number;
  vx: number;
  vy: number;
  size: number;
  targetX?: number;
  targetY?: number;
  alpha: number;
  isHero?: boolean;
  isAgent?: boolean;
}

const HeroAnimation: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    let animationFrameId: number;
    let width = 0;
    let height = 0;
    let isMobile = false;
    let particles: Particle[] = [];
    let phase: 'FLOW' | 'SPARK' | 'CONNECT' | 'FORM' | 'GUARD' | 'CHAT' = 'FLOW';
    let labelText = "";
    let startTime = Date.now();

    // Configuration
    const PRIMARY_COLOR = '#FF5500';

    const resize = () => {
      width = canvas.width = canvas.parentElement?.offsetWidth || window.innerWidth;
      height = canvas.height = canvas.parentElement?.offsetHeight || window.innerHeight;
      isMobile = width < 768;
      initParticles();
    };

    const initParticles = () => {
      particles = [];
      // Reduce count on mobile to reduce visual noise behind text
      const count = isMobile ? 30 : 60;
      
      for (let i = 0; i < count; i++) {
        particles.push({
          x: Math.random() * width,
          y: Math.random() * height,
          vx: (Math.random() + 0.2) * (isMobile ? 1 : 2), 
          vy: (Math.random() - 0.5) * 0.5,
          size: Math.random() * 2 + 1,
          alpha: Math.random() * 0.5 + 0.1,
          isHero: i === 0, // First particle is the Hero
          isAgent: i < 6 // First 6 particles will form the Agent/Logo
        });
      }
    };

    const drawLabel = (cx: number, cy: number, text: string, progress: number) => {
        if (!text) return;
        ctx.save();
        ctx.translate(cx, cy + 160); // Position below the main action
        ctx.globalAlpha = Math.min(1, Math.sin(progress * Math.PI)); // Fade in/out
        
        ctx.font = "10px 'SF Mono', 'Menlo', monospace";
        ctx.textAlign = "center";
        ctx.fillStyle = PRIMARY_COLOR;
        ctx.shadowColor = PRIMARY_COLOR;
        ctx.shadowBlur = 10;
        
        // Typing effect simulation
        const chars = Math.floor(text.length * Math.min(1, progress * 3));
        const currentText = text.substring(0, chars);
        
        ctx.fillText(`[ ${currentText} ]`, 0, 0);
        
        // Underline decoration
        ctx.strokeStyle = `rgba(255, 255, 255, 0.2)`;
        ctx.beginPath();
        ctx.moveTo(-20, 15);
        ctx.lineTo(20, 15);
        ctx.stroke();

        ctx.restore();
    };

    const drawChatInterface = (cx: number, cy: number, progress: number) => {
      ctx.save();
      ctx.translate(cx, cy);
      
      const w = isMobile ? 160 : 200;
      const h = isMobile ? 200 : 260;
      const scale = Math.min(1, Math.max(0.8, progress * 2));
      
      ctx.scale(scale, scale);
      
      // Window Frame - Thinner lines for elegance
      ctx.strokeStyle = `rgba(255, 255, 255, ${0.1 + Math.min(0.2, progress)})`;
      ctx.lineWidth = 0.5; 
      ctx.fillStyle = 'rgba(5, 5, 5, 0.8)'; // Darker background to pop against lines
      
      // Draw Box
      ctx.beginPath();
      // @ts-ignore
      if (ctx.roundRect) {
         ctx.roundRect(-w/2, -h/2, w, h, 12);
      } else {
         ctx.rect(-w/2, -h/2, w, h);
      }
      ctx.fill();
      ctx.stroke();

      // Header Line
      ctx.beginPath();
      ctx.moveTo(-w/2, -h/2 + 30);
      ctx.lineTo(w/2, -h/2 + 30);
      ctx.stroke();

      // UI Dots
      ctx.fillStyle = 'rgba(255, 255, 255, 0.2)';
      ctx.beginPath();
      ctx.arc(-w/2 + 20, -h/2 + 15, 2, 0, Math.PI*2);
      ctx.arc(-w/2 + 32, -h/2 + 15, 2, 0, Math.PI*2);
      ctx.fill();

      // Chat Bubbles Animation
      const timeInChat = progress * 5; 
      
      const bubbles = [
        { start: 0.5, type: 'user', width: w * 0.6, height: 24 },
        { start: 1.5, type: 'system', width: w * 0.7, height: 40 },
        { start: 3.5, type: 'user', width: w * 0.4, height: 24 },
        { start: 4.5, type: 'typing', width: 40, height: 20 },
      ];

      bubbles.forEach((b, i) => {
         if (timeInChat > b.start) {
             const isUser = b.type === 'user';
             const xPos = isUser ? (w/2 - 20 - b.width) : (-w/2 + 20);
             const yPos = -h/2 + 50 + (i * 50);
             
             // Bubble Rect
             ctx.fillStyle = isUser ? 'rgba(255, 255, 255, 0.08)' : 'rgba(255, 85, 0, 0.1)';
             ctx.beginPath();
             if (ctx.roundRect) {
                 ctx.roundRect(xPos, yPos, b.width, b.height, 6);
             } else {
                 ctx.rect(xPos, yPos, b.width, b.height);
             }
             ctx.fill();

             // Typing Indicator
             if (b.type === 'typing') {
                 const dotTime = Date.now() / 200;
                 ctx.fillStyle = PRIMARY_COLOR;
                 for(let d=0; d<3; d++) {
                     const active = Math.floor(dotTime) % 3 === d;
                     ctx.globalAlpha = active ? 1 : 0.3;
                     ctx.beginPath();
                     ctx.arc(xPos + 12 + (d*8), yPos + 10, 1.5, 0, Math.PI*2);
                     ctx.fill();
                 }
                 ctx.globalAlpha = 1;
             }
         }
      });

      ctx.restore();
    };

    const render = () => {
      const now = Date.now();
      let elapsed = now - startTime;
      const LOOP_DURATION = 18000;

      // Loop Logic
      if (elapsed > LOOP_DURATION) {
        startTime = now;
        elapsed = 0;
        phase = 'FLOW';
        initParticles(); 
      }

      // State Machine & Narrative Logic
      if (elapsed < 2000) { phase = 'FLOW'; labelText = "AWAITING INPUT"; }
      else if (elapsed < 4000) { phase = 'SPARK'; labelText = "DETECTING SIGNAL"; }
      else if (elapsed < 6500) { phase = 'CONNECT'; labelText = "ESTABLISHING CONTEXT"; }
      else if (elapsed < 9000) { phase = 'FORM'; labelText = "ISOLATING ENVIRONMENT"; }
      else if (elapsed < 11500) { phase = 'GUARD'; labelText = "APPLYING GUARDRAILS"; }
      else { phase = 'CHAT'; labelText = "AGENT ACTIVE"; }

      // Clear Screen
      ctx.clearRect(0, 0, width, height);
      
      // Determine center
      // On Mobile: keep it centered, but maybe shifted down slightly if needed.
      // On Desktop: shift to right.
      const cx = width > 1024 ? width * 0.75 : width / 2;
      const cy = height / 2;

      // Update & Draw Particles
      particles.forEach((p, i) => {
        // --- Physics Update ---
        if (phase === 'FLOW' || phase === 'SPARK' || (phase === 'CONNECT' && !p.isHero)) {
          p.x += p.vx;
          p.y += p.vy;
          // Wrap around
          if (p.x > width) p.x = 0;
          if (p.y > height) p.y = 0;
          if (p.y < 0) p.y = height;
        } 
        else if (phase === 'FORM' || phase === 'GUARD' || phase === 'CHAT') {
          // Morph to target shape using Lerp for smooth "effortless" physics
          if (p.isAgent) {
            let tx = cx;
            let ty = cy;
            const w = isMobile ? 160 : 200;
            const h = isMobile ? 200 : 260;
            
            // Map particles to key points of the window
            if (i === 0) { tx = cx - w/2; ty = cy - h/2; } // Top Left
            else if (i === 1) { tx = cx + w/2; ty = cy - h/2; } // Top Right
            else if (i === 2) { tx = cx + w/2; ty = cy + h/2; } // Bot Right
            else if (i === 3) { tx = cx - w/2; ty = cy + h/2; } // Bot Left
            else if (i === 4) { tx = cx; ty = cy - h/2; } // Top Mid
            else if (i === 5) { tx = cx; ty = cy + h/2; } // Bot Mid
            
            // Smooth ease
            p.x += (tx - p.x) * 0.08;
            p.y += (ty - p.y) * 0.08;
          } else {
             // Others drift away or fade
             p.alpha *= 0.95;
          }
        }

        // --- Visuals ---
        
        // Spark Phase: Hero brightens
        if (phase === 'SPARK' && p.isHero) {
           p.x += (cx - p.x) * 0.05;
           p.y += (cy - p.y) * 0.05;
           
           ctx.shadowBlur = 15;
           ctx.shadowColor = PRIMARY_COLOR;
           ctx.fillStyle = PRIMARY_COLOR;
           ctx.beginPath();
           ctx.arc(p.x, p.y, p.size * 2, 0, Math.PI * 2);
           ctx.fill();
           ctx.shadowBlur = 0;
        }

        // Connect Phase: Draw THIN lines
        if (phase === 'CONNECT' || phase === 'FORM') {
            if (p.isHero) {
                particles.forEach(other => {
                    const dist = Math.hypot(p.x - other.x, p.y - other.y);
                    if (dist < (isMobile ? 150 : 300)) {
                        ctx.beginPath();
                        ctx.moveTo(p.x, p.y);
                        ctx.lineTo(other.x, other.y);
                        // Very thin lines for elegance
                        ctx.lineWidth = isMobile ? 0.2 : 0.5;
                        ctx.strokeStyle = `rgba(255, 85, 0, ${1 - dist/(isMobile ? 150 : 300)})`;
                        ctx.stroke();
                    }
                });
            }
        }

        // Form/Guard Phase: Draw Box Geometry
        if ((phase === 'FORM' || phase === 'GUARD') && p.isAgent) {
             particles.filter(n => n.isAgent && n !== p).forEach(neighbor => {
                 ctx.beginPath();
                 ctx.moveTo(p.x, p.y);
                 ctx.lineTo(neighbor.x, neighbor.y);
                 ctx.strokeStyle = `rgba(255, 255, 255, 0.15)`;
                 ctx.lineWidth = 0.5;
                 ctx.stroke();
             });
        }
        
        // Guard Phase: Outer Shell
        if (phase === 'GUARD') {
            const size = isMobile ? 130 : 160;
            ctx.save();
            ctx.translate(cx, cy);
            ctx.strokeStyle = `rgba(255, 255, 255, ${Math.abs(Math.sin(elapsed / 200)) * 0.4 + 0.1})`;
            ctx.lineWidth = 1;
            ctx.setLineDash([10, 10]);
            ctx.beginPath();
            ctx.rect(-size/2, -size/2, size, size);
            ctx.stroke();
            
            // Scanning effect
            ctx.fillStyle = 'rgba(255, 85, 0, 0.05)';
            ctx.fillRect(-size/2, -size/2 + (elapsed % 1000)/1000 * size, size, 2);
            ctx.restore();
        }

        // Chat Phase: Draw Interface
        if (phase === 'CHAT') {
           drawChatInterface(cx, cy, Math.min(1, (elapsed - 11500) / 1000));
        }

        // Draw particle dot
        // On mobile, reduce alpha generally to avoid text conflict
        const mobileAlphaMod = isMobile ? 0.4 : 1;
        
        ctx.fillStyle = p.isHero || (phase !== 'FLOW' && p.isAgent) ? PRIMARY_COLOR : '#fff';
        if (phase === 'CHAT' && p.isAgent) ctx.fillStyle = '#fff';
        
        ctx.globalAlpha = p.alpha * mobileAlphaMod;
        ctx.beginPath();
        // Smaller particles on mobile
        ctx.arc(p.x, p.y, p.size * (isMobile ? 0.7 : 1), 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
      });

      // Draw Narrative Label
      // Only draw if we are in an active phase (not just flow)
      if (phase !== 'FLOW') {
          // Progress calculation for label fade
          const phaseStartTimes = { SPARK: 2000, CONNECT: 4000, FORM: 6500, GUARD: 9000, CHAT: 11500 };
          const phaseDuration = 2500;
          // @ts-ignore
          const currentPhaseStart = phaseStartTimes[phase] || 0;
          const progress = Math.min(1, Math.max(0, (elapsed - currentPhaseStart) / phaseDuration));
          
          drawLabel(cx, cy, labelText, progress);
      }

      animationFrameId = requestAnimationFrame(render);
    };

    // Initialize
    resize();
    window.addEventListener('resize', resize);
    render();

    return () => {
      window.removeEventListener('resize', resize);
      cancelAnimationFrame(animationFrameId);
    };
  }, []);

  return (
    <div className="w-full h-full relative">
      <canvas ref={canvasRef} className="w-full h-full block" />
    </div>
  );
};

export default HeroAnimation;
